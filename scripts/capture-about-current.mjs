...
async function captureDesktopCurrent(browser, targetUrl) {
...
async function captureStaticDesktopCurrent(browser, targetUrl) {
  const context = await browser.newContext({
    colorScheme: "dark",
    reducedMotion: "no-preference",
...
async function captureMobileCurrent(browser, targetUrl) {
  const context = await browser.newContext({
    colorScheme: "dark",
    reducedMotion: "no-preference",
...
async function main() {
  ...
  try {
    const savedPaths = [];
    savedPaths.push(...(await captureDesktopCurrent(browser, targetUrl)));
    savedPaths.push(...(await captureStaticDesktopCurrent(browser, targetUrl)));
    savedPaths.push(...(await captureMobileCurrent(browser, targetUrl)));

    console.log(`Captured about current snapshots from ${targetUrl}:`);
