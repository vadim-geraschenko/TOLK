...
## Button System
- В текущем MVP базовый тип кнопки один: `.button`.
- Отдельный рабочий визуальный тип `.button-primary` сейчас не используется и не должен возвращаться без отдельного решения.
- `.mini-button` — не отдельная кнопка, а size-modifier поверх `.button`.
- Базовая кнопка:
  - серебристо-графитовая в rest-state;
  - на hover слегка приподнимается и немного увеличивается;
  - уходит в более глубокий чёрно-серебряный фон;
  - содержит внутренний слой маленьких серебряных звёзд.
- Звёзды:
  - в текущей реализации их `7`;
  - анимируются только как часть hover-образа кнопки;
  - на ширине окна меньше примерно `854px` получают отдельную раскладку по ширине кнопки.
- Если править кнопки дальше, сначала сохранять единый visual contract `.button`, а уже потом вводить варианты.
